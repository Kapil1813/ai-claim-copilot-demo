...
def extract_codes(note):
    ...
    prompt = f"""
Extract ICD-10 and CPT codes from the clinical note.
Return JSON as:
...
"""
    try:
        ...
        return (
            result.get("icd", "E11.9"),
            result.get("cpt", ["83036"]),
            result.get("confidence", 85),
            result.get("explanation", "No explanation provided.")
        )
    except Exception as e:
        st.error(f"OpenAI API failed: {e}")
        # Fallback
        return "E11.9", ["83036"], 80, "Fallback explanation used."
...
if st.session_state.ai_cpt:
    summary_list = []
    for c in st.session_state.ai_cpt:
        summary_list.append({
            "CPT Code": c,
            "Procedure": CPT_REFERENCE.get(c, ""),
            "Avg Denial Risk": f"{calculate_denial_risk(c)}%"
        })
    st.subheader("Average Denial Risk by CPT Code")
    st.table(pd.DataFrame(summary_list))

# ---------------------------
# Editable ICD/CPT before submission
# ---------------------------
if st.session_state.ai_icd:
    ...
    if submit:
        ...
        st.success("✅ Claim added to Claim Predictions Table")
...
# Privacy Notice
# ---------------------------
st.markdown("⚠️ **Demo only. Do not enter real patient data.**")